...
export default class Issue extends React.Component {
  ...
  render() {
    ...
    const closedAtDate = new Date(
      this.props.issue.closed_at
    ).toLocaleDateString();

    return (
      <div className="issue card mb-4">
        <h4>
          <a href={`${this.props.issue.html_url}`}>{this.props.issue.title}</a>
        </h4>
        <div className="text-muted">
          ...
          <img
            ...
            alt={this.props.issue.user.login}
          />
        </div>
        ...
        <div className="issue-more-button">
          {this.props.issue.body ? (
            this.state.detailed ? (
              <FontAwesomeIcon onClick={this.showDetailed} icon="chevron-up" />
            ) : (
              <FontAwesomeIcon
                onClick={this.showDetailed}
                icon="chevron-down"
              />
            )
          ) : (
